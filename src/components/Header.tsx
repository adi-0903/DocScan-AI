...
import { Sparkles, Scan, Database, User as UserIcon, LogOut, ChevronDown, Crown, Sun, Moon, Shield } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const isAdmin = currentUser?.email?.toLowerCase() === 'singhaladitya611@gmail.com';
      ...
      <div className="max-w-4xl mx-auto flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center shadow-md shadow-indigo-500/20 text-white shrink-0">
            {isAdmin ? <Shield className="w-5 h-5 text-amber-300" /> : <Scan className="w-4 h-4" />}
          </div>
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <h1 className="text-xs sm:text-sm font-bold tracking-tight text-white truncate">
                {isAdmin ? 'DocScan AI - Admin Portal' : 'DocScan AI'}
              </h1>
              {isAdmin ? (
                <span className="inline-flex items-center gap-1 text-[9px] sm:text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-400/20 text-amber-300 border border-amber-400/30 shrink-0">
                  SYSTEM ADMIN
                </span>
              ) : (
            ...
            <p className="text-slate-400 text-[10px] sm:text-[11px] truncate">
              {isAdmin ? 'User Management & Plan Activation' : 'Field Extraction Assistant'}
            </p>
          </div>
        </div>
          ...
          {currentUser ? (
            <div className="relative">
              ...
              >
                <div className="w-5 h-5 rounded-full bg-indigo-600 flex items-center justify-center text-[10px] font-bold text-white">
                  {isAdmin ? <Shield className="w-3 h-3 text-amber-300" /> : currentUser.name.charAt(0).toUpperCase()}
                </div>
                <span className="font-semibold text-[11px] max-w-[100px] truncate hidden xs:inline">
                  {isAdmin ? 'Admin Portal' : currentUser.name.split(' ')[0]}
                </span>
              ...
              {showDropdown && (
                <>
                  ...
                  />
                  <div className="absolute right-0 mt-1 w-52 bg-slate-900 border border-slate-800 rounded-xl shadow-xl z-20 p-2 space-y-1">
                    <div className="px-2 py-1.5 border-b border-slate-800">
                      <p className="text-xs font-bold text-white truncate">{currentUser.name}</p>
                      <p className="text-[10px] text-slate-400 truncate">{currentUser.email}</p>
                      {isAdmin && (
                        <span className="inline-block text-[9px] bg-amber-400/20 text-amber-300 border border-amber-400/30 px-2 py-0.5 rounded-full font-bold mt-1">
                          MASTER ADMIN
                        </span>
                      )}
                    </div>
                    ...
                    <button
                      ...
                      className="w-full text-left px-2 py-1.5 rounded-lg text-xs font-bold text-red-400 hover:bg-slate-800 hover:text-red-300 flex items-center gap-1.5 transition-colors border-t border-slate-800/80 pt-1.5 mt-1"
                    >
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="px-2.5 py-1 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-xs active:scale-95 transition-all"
              >
                Log In
              </button>
            </div>
